...
def is_main():  # 仅 rank0 存档时会有用；你也可以不改存档逻辑
    return (not dist.is_initialized()) or dist.get_rank() == 0
__PREFERRED_DEVICE__ = torch.device(f"cuda:{LOCAL_RANK if LOCAL_RANK!=-1 else 0}")  # 代替原来的 cuda:0
...
if torch.cuda.is_available():
    # 如果被 torchrun 调起，会在 init_distributed() 里把设备设为 cuda:LOCAL_RANK
    __PREFERRED_DEVICE__ = torch.device(f"cuda:{LOCAL_RANK if LOCAL_RANK != -1 else 0}")
else:
...
class Pipeline:
    ...
    def get_encoded_latent(self, cloud_id):
        return torch.load(self.workplace +f"/encoded/latent_{cloud_id}.pt", map_location=self.get_device(), weights_only=False)

    def get_enhanced_latent(self, latent_id):
        return torch.load(self.workplace +f"/enhanced/latent_{latent_id}.pt", map_location=self.get_device(), weights_only=False)

    def get_test_latent(self, latent_id):
        return torch.load(self.workplace +f"/test/latent_{latent_id}.pt", map_location=self.get_device(), weights_only=False)
    ...
    def get_latents_for_decoder_training(self, cp: typing.Optional[int] = None):
        ...
        return torch.load(self.workplace+f"/decoder/latents_{cp}.pt", map_location=self.get_device(), weights_only=False)

    def get_volume_for_decoder_training(self, id: int):
        try:
            ids = torch.load(self.workplace + "/decoder/batch_used.pt", weights_only=False)
        except:
            raise Exception("No trained decoder yet.")
        return self.from_grid_to_volume(torch.load(
                self.get_clouds_path() + f"/cloud_{ids[id]}.pt", map_location=self.get_device(), weights_only=False
            ))
    ...
    def get_decoder(self, cp: typing.Optional[int] = None):
        if not hasattr(self, 'decoding'):
            ...
            if cp > 0:  # some checkpoint found
                decoder.load_state_dict(torch.load(self.workplace+f"/decoder/decoder_{cp}.pt", weights_only=False))
                upsampler.load_state_dict(torch.load(self.workplace + f"/decoder/upsampler_{cp}.pt", weights_only=False))
    ...
    def get_diffuser(self, cp: typing.Optional[int] = None, use_ema: bool = True):
        if not hasattr(self, 'diffuser'):
            ...
            if cp > 0:  # some checkpoint found
                if use_ema:
                    diffuser.denoiser.load_state_dict(torch.load(self.workplace + f"/diffuser/denoiser_{cp}.pt", weights_only=False))
                else:
                    diffuser.load_state_dict(torch.load(self.workplace + f"/diffuser/optimizing_{cp}.pt", weights_only=False)['model'])
    ...
    def get_normalization_stats(self):
        if not hasattr(self, 'norm_stats'):
            self.norm_stats = torch.load(self.workplace + "/enhanced/stats.pt", map_location=self.get_device(), weights_only=False)
        return self.norm_stats

    def load_cloud_grid(self, cloud_id):
        return torch.load(self.get_clouds_path() + f"/cloud_{cloud_id}.pt", map_location=preferred_device(), weights_only=False)
    ...
    def run_train_decoder_batched(self):
        ...
        ids = torch.load(output_path + "/batch_used.pt", weights_only=False)
        # get decoder
        decoder, upsampler, cp = self.get_decoder()
        if LOCAL_RANK != -1:
            decoder   = DDP(decoder,   device_ids=[LOCAL_RANK], output_device=LOCAL_RANK)
            upsampler = DDP(upsampler, device_ids=[LOCAL_RANK], output_device=LOCAL_RANK)
        ...
        if cp > 0:
            with torch.no_grad():
                latents.copy_(torch.load(self.workplace + f"/decoder/latents_{cp}.pt", weights_only=False))
        ...
        if cp > 0:  # load optimizer states if checkpoint exists
            decoder_opt.load_state_dict(torch.load(self.workplace+f"/decoder/decoder_opt_{cp}.pt", weights_only=False))
            latents_opt.load_state_dict(torch.load(self.workplace+f"/decoder/latents_opt_{cp}.pt", weights_only=False))
        ...
        for step in steps_iterator:
            ...
            if (step + 1) % cp_every == 0:  # save checkpoint
                ...
                torch.save(unwrap(decoder).state_dict(), self.workplace +f"/decoder/decoder_{current_cp}.pt")
                torch.save(unwrap(upsampler).state_dict(), self.workplace +f"/decoder/upsampler_{current_cp}.pt")
    ...
    def run_compute_normalization_stats(self):
        ...
        if rep_mode == RepresentationModes.monoplanar_128_32:
            ...
            while os.path.exists(
                ...
                l = torch.load(augmented_dataset_path + f"/latent_{cloud_id}.pt", map_location=self.get_device(), weights_only=False)
    ...
    def run_train_diffuser(self):
            ...
            with torch.no_grad():
                for i, id in enumerate(ids):
                    images[i].copy_(torch.load(augmented_dataset_path + f"/latent_{id}.pt",
                                               map_location=self.get_device(), weights_only=False).permute(2, 0, 1))  # channels first
        ...
        if cp > 0:
            if os.path.exists(f"{result_path}/optimizer_{cp}.pt"):
                optimizer.load_state_dict(torch.load(f"{result_path}/optimizer_{cp}.pt", weights_only=False))
    ...
    def run_train_diffuser_acc(self):
        ...
        if cp > 0:
            if os.path.exists(f"{result_path}/optimizing_{cp}.pt"):
                opt_data = torch.load(f"{result_path}/optimizing_{cp}.pt", weights_only=False)
